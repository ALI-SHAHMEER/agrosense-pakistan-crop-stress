...
GEE data collection pipeline, Jupyter notebook, and pre-trained models are
...
SVM~(RBF) achieving the highest test accuracy (98.34\%) and XGBoost achieving
the best cross-validated macro F1 (0.983) under field-grouped evaluation.
Second, field-level train/test separation (\texttt{GroupShuffleSplit} on site
identifiers) is essential for honest accuracy estimation when datasets contain
multiple seasonal observations per location; naive random splits inflate
reported accuracy by allowing the same site to appear in both partitions.
Third, a single stateless REST API deployed on Google Cloud
Run~\cite{gorelick2017,fastapi} can serve both a Flutter mobile
